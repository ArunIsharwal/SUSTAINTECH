...
//             ].map((item) => (
//               <div key={item.label} className="flex items-center gap-3 bg-white/10 backdrop-blur-sm rounded-xl px-4 py-3">
...
//                       <role.icon className={`w-6 h-6 ${formData.role === role.id ? "text-primary" : "text-muted-foreground"}`} />
//                       <span className={`text-sm font-medium ${formData.role === role.id ? "text-primary" : "text-foreground"}`}>
//                         {role.label}
//                       </span>
//                     </Label>
//                   ))}
...
//   // ✅ ONLY THIS FUNCTION WAS CHANGED (OTP LOGIC)
//   const handleSubmit = async (e: React.FormEvent) => {
//     e.preventDefault();
//     setIsLoading(true);
